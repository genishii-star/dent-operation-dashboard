...
// AI Chat
// ============================================================
const GAS_AI_URL = 'https://script.google.com/macros/s/AKfycbzr0rFEWTEV1tUZnVO1u0Wz6VoWsYtb2Z37Yn1F5GcN9Ed9s88McT_jVZVamdP5j0e7-w/exec';
...
function clearAiChat() {
  ...
  container.innerHTML = `<div class="ai-msg ai-msg-assistant">
    こんにちは！民泊運営に関するご質問をどうぞ。<br>
    物件のパフォーマンスや価格設定、市場データについて相談できます。
  </div>`;
}

function buildAiContext() {
  const lines = [];
  lines.push('## 会社概要\nDent Inc. — 民泊（短期賃貸）運営代行。大阪・京都・東京を中心に展開。');
  lines.push(`現在の日付: ${new Date().toISOString().slice(0, 10)}`);

  if (properties.length) {
    lines.push(`\n## 管理物件一覧 (${properties.length}件)`);
    properties.forEach(p => {
      const parts = [p.name];
      if (p.area) parts.push(p.area);
      if (p.rooms) parts.push(p.rooms + '室');
      if (p.ownerName) parts.push('オーナー: ' + p.ownerName);
      if (p.licenseType) parts.push(p.licenseType);
  ...
  if (reservations.length) {
    ...
    const monthRes = reservations.filter(r => r.checkin && r.checkin.startsWith(thisMonth));
    lines.push(`\n## 今月の予約概要 (${thisMonth})`);
    lines.push(`- 全予約数: ${reservations.length}件`);
    lines.push(`- 今月チェックイン: ${monthRes.length}件`);

    const propPerf = {};
    reservations.forEach(r => {
      if (!r.propertyName) return;
      if (!propPerf[r.propertyName]) propPerf[r.propertyName] = { count: 0, revenue: 0 };
      propPerf[r.propertyName].count++;
      propPerf[r.propertyName].revenue += (parseFloat(r.totalAmount) || 0);
    });

    lines.push('\n## 物件別パフォーマンス (全期間)');
    Object.entries(propPerf)
      .sort((a, b) => b[1].revenue - a[1].revenue)
      .slice(0, 20)
      .forEach(([name, d]) => {
        lines.push(`- ${name}: ${d.count}件, 売上¥${Math.round(d.revenue).toLocaleString()}`);
      });
  }

  const adSheets = window._airdnaSheets || {};
  if (Object.keys(adSheets).length) {
    lines.push('\n## AirDNA市場データ');
    const latest = (sheet, fields) => {
      ...
      for (const f of fields) {
        ...
        if (idx >= 0) {
          for (let i = sheet.length - 1; i >= 1; i--) {
            const v = parseFloat(sheet[i][idx]);
            if (!isNaN(v)) return { month: sheet[i][0], value: v };
          }
        }
      }
      return null;
    };
    ['大阪', '京都', '東京'].forEach(city => {
      const occSheet = adSheets[`AD_${city}全域_occupancy`];
      const rateSheet = adSheets[`AD_${city}全域_rates_summary`];
      const revSheet = adSheets[`AD_${city}全域_revenue_summary`];
      if (!occSheet && !rateSheet && !revSheet) return;
      lines.push(`\n### ${city}`);
      const occ = latest(occSheet, ['Occupancy rate', 'Occupancy', 'occupancy_rate']);
      const adr = latest(rateSheet, ['Average daily rate', 'Daily rate', 'Rate', 'daily_rate']);
      const rev = latest(revSheet, ['Average annual revenue', 'Revenue', 'revenue']);
      if (occ) lines.push(`- OCC: ${(occ.value).toFixed(1)}% (${occ.month})`);
      if (adr) lines.push(`- ADR: ¥${Math.round(adr.value).toLocaleString()} (${adr.month})`);
      if (rev) lines.push(`- Revenue: ¥${Math.round(rev.value).toLocaleString()} (${rev.month})`);
    });
  }
...
const AI_SYSTEM_PROMPT = `あなたはDent Inc.の民泊運営アドバイザーAIです。

## 役割
- 民泊（短期賃貸）の運営に関する質問に回答する
- 物件のパフォーマンス分析と改善提案を行う
- 価格設定（ADR）について具体的な金額を提案する
- 市場データを踏まえた戦略的アドバイスを提供する

## 回答スタイル
- 簡潔で実用的な回答（長くても300文字程度）
- 数字を使って具体的に提案する（「¥XX,XXX に値上げ」「OCC XX% を目標に」等）
- 箇条書きを活用して読みやすくする
- 不確実な場合は正直に伝え、追加データが必要ならその旨を伝える

## コンテキスト
以下にダッシュボードの現在のデータが含まれています。このデータを参照して回答してください。`;
...
async function sendAiChat() {
  ...
  if (!GAS_AI_URL) {
    appendAiMessage('assistant', 'AI機能を利用するにはGASプロキシのデプロイが必要です。gas-ai-proxy.js のセットアップ手順を確認してください。');
    return;
  }
  ...
  const thinkingDiv = appendAiMessage('thinking', '考え中...');
  ...
  try {
    ...
    const resp = await fetch(GAS_AI_URL, {
      method: 'POST',
    ...
    if (data.error) {
      appendAiMessage('assistant', 'エラーが発生しました: ' + data.error);
    } else {
    ...
    thinkingDiv.remove();
    appendAiMessage('assistant', '通信エラー: ' + e.message);
  } finally {
